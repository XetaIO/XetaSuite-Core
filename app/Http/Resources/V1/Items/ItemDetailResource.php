...
class ItemDetailResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'price_history' => $this->whenLoaded('prices', fn() =>
            ItemPriceResource::collection($this->prices)
            ),

            // History of movements
            'movement_history' => $this->whenLoaded('movements', fn() =>
            ItemMovementResource::collection($this->movements)
            ),

            // Linked materials
            'materials' => $this->whenLoaded('materials', fn() =>
            $this->materials->map(fn($material) => [
                'id' => $material->id,
                'name' => $material->name,
            ])
            ),
